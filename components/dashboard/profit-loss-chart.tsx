...
export function ProfitLossChart() {
  ...
  useEffect(() => {
    if (bets.length > 0) {
      const data = processChartData(bets)
      setChartData(data)
  ...
  useEffect(() => {
    ...
    const animatedPoints = chartData.map((data, i) => {
      const x = padding + (chartWidth / (chartData.length - 1)) * i
      const targetY = getY(data.profit)
      ...
      for (const point of animatedPoints) {
        ...
        if (distance < 15) {
          ...
          setTooltip({
            ...
            x: point.x,
            y: point.y,
            content: `<strong>Data:</strong> ${point.data.date}<br><strong>Lucro/Prejuízo:</strong> ${formatCurrency(point.data.profit)}`,
          })
          isOverPoint = true
          break
        }
...
function processChartData(bets: any[]) {
